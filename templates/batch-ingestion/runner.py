#!/usr/bin/env python3
"""
templates/batch-ingestion/runner.py

A production-minded batch ingestion runner that:
- Loads a pipeline config (YAML)
- Reads source files from S3 or local filesystem
- Applies a simple transform (CSV -> Parquet) with schema detection
- Writes results to a destination (S3 or local filesystem)
- Implements retries, logging, and idempotency checks

Usage:
    python templates/batch-ingestion/runner.py --config templates/batch-ingestion/pipeline.yaml

Config example (pipeline.yaml):
---
name: example-batch-ingest
source:
  type: s3               # or "local"
  s3_bucket: my-bucket
